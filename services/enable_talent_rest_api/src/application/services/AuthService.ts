...
export class AuthService {
   ...
   * Sign up a new user
   */
  async signup(data: unknown): Promise<AuthResponse> {
    ...
    const { data: authData, error: authError } = await this.fastify.supabase.auth.signUp({
      email,
      password,
      options: {
        data: {
  ...
  async login(data: unknown): Promise<AuthResponse> {
    ...
    const { data: authData, error: authError } =
      await this.fastify.supabase.auth.signInWithPassword({
        email,
        password,
      });
