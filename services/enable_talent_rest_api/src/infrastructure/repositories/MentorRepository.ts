...
export async function listMentors(
  ...
  const result = await fastify.pg.query<MentorCard>(query, params);

  // Get total count
  const countResult = await fastify.pg.query<{ count: string }>(
 ...
 */
export async function findMentorById(
  fastify: FastifyInstance,
  mentorId: string
): Promise<Mentor | null> {
  const mentor = await fastify.pg.query<any>(
    `SELECT
      u.user_id,
      u.name,
      u.email,
  ...
  // Get skills, experience, education in parallel
  const [skills, experience, education] = await Promise.all([
    fastify.pg.query<{ skill_id: string; skill_name: string }>(
...
export async function isMentor(
  ...
  const result = await fastify.pg.query<{ usertype_id: string }>(
